...
export default function Home(){
 ...
 <section className="hero"><div className="heroShade"></div><div className="heroCopy scroll-reveal"><h1><span>MORE JOBS.</span><br/>LESS CHASING.</h1><h2>Digital marketing built for tradies.</h2><p>We bring you high-quality leads so you<br/>can get on with what you do best.</p><a className="pinkBtn heroBtn" href="#contact">GET MORE JOBS&nbsp; →</a></div></section>
 <div className="tradebar scroll-reveal">{[["⌂","ROOFING"],["⌕","PLUMBING"],["ϟ","ELECTRICAL"],["❄","HVAC"],["♜","FENCING"],["▦","BUILDING"],["◒","LANDSCAPING"]].map(([i,t])=><div key={t}><b>{i}</b>{t}</div>)}</div>
 ...
 <section id="contact" className="leadSection"><div className="phoneVisual scroll-reveal"><div className="phone"><small>Incoming Call</small><b>New Lead</b><span>Monsta Miami</span><div>● &nbsp;&nbsp; ●</div></div></div><div className="leadCopy scroll-reveal"><small>LET’S SEE IF WE CAN GET YOU</small><h3>MORE JOBS.</h3><p>Tell us your trade and where you work.<br/>We’ll show you exactly what we’d do<br/>to get you more jobs.</p><span className="curve">↘</span></div><form className="scroll-reveal"><select defaultValue=""><option value="" disabled>Your Trade</option>{trades.map(t=><option key={t}>{t}</option>)}</select><input placeholder="Suburb / Area"/><input placeholder="Phone Number"/><button type="button">SHOW ME THE OPPORTUNITY&nbsp; →</button><small>▣ No spam. No obligation.</small></form></section>
 <section id="about" className="process"><div className="scroll-reveal"><small>OUR PROCESS</small><h3>SIMPLE. PROVEN. BUILT FOR TRADIES.</h3></div><div className="scroll-reveal reveal-stagger">{[["▤","1. ENQUIRY","Tell us about your trade and where you work."],["▧","2. PLAN","We show you the best strategy for your area."],["➤","3. LAUNCH","We set everything up and get it live."],["▥","4. MORE JOBS","You get consistent, high-quality enquiries."]].map(([i,n,d])=><article key={n}><b>{i}</b><h4>{n}</h4><p>{d}</p></article>)}</div></section>
 <section className="finalCta"><div className="roof scroll-reveal"></div><div className="scroll-reveal"><small>READY TO GET STARTED?</small><h3>MORE JOBS<br/>ARE OUT THERE.</h3><p>Let’s get you in front of the right customers<br/>and turn more enquiries into real work.</p><a className="pinkBtn" href="#contact">GET MORE JOBS&nbsp; →</a></div></section>
 </main>
}