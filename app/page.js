 ...
 useEffect(()=>{
  ...
  const update=()=>{
   ...
   sections.forEach((section)=>{
    ...
    const distance=(center-vh/2)/(vh+r.height);
    const progress=Math.max(-1,Math.min(1,distance));
    const visible=Math.max(0,Math.min(1,(vh-r.top)/(Math.min(vh,r.height)*0.7)));
    section.style.setProperty("--section-progress",progress.toFixed(4));
    section.style.setProperty("--section-visible",visible.toFixed(4));
   });
   raf=0;
  };
  ...
  update();
  return()=>{window.removeEventListener("scroll",requestUpdate);window.removeEventListener("resize",requestUpdate);if(raf)cancelAnimationFrame(raf);};
 },[]);
